...
Program::Program(const ProgramDescription& description, const char* vertex, const char* fragment) {
    /// M: @{
    ATRACE_NAME("Program by building");
...
Program::~Program() {
    if (mInitialized) {
        ...
        */
        // Avoid detach and delete zero shader. Because shader won't be
        // initialized due to our program binary enhancement.
        bool hasProgramAtlas = (ExtensionUtils::getProgramAtlas() != NULL);
        if ((hasProgramAtlas && mVertexShader != 0) || !hasProgramAtlas) {
            glDetachShader(mProgramId, mVertexShader);
            glDeleteShader(mVertexShader);
        }
        if ((hasProgramAtlas && mFragmentShader != 0) || !hasProgramAtlas) {
            glDetachShader(mProgramId, mFragmentShader);
            glDeleteShader(mFragmentShader);
        }
        /// @}
