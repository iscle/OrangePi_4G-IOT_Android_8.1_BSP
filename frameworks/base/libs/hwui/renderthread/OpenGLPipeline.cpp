...
bool OpenGLPipeline::draw(const Frame& frame, const SkRect& screenDirty, const SkRect& dirty,
    ...
    frameBuilder.setDisplayListRecorderHost(this);
#endif
...
#ifdef ENABLE_DR_HWUI
    RECORD_FRAME(frameBuilder.deferRenderNodeScene(renderNodes, contentDrawBounds));
#else
...
bool OpenGLPipeline::swapBuffers(const Frame& frame, bool drew, const SkRect& screenDirty,
    ...
    GL_CHECKPOINT(LOW);
    */
    // Error handling for surface invalid case.
    // Surface may be disconnected under normal flow related to timing of WMS handling.
    // Do not assert when surface is invalid due to disconnection.
    ...
    */
    if (*requireSwap && (CC_UNLIKELY(!isSurfaceValid
