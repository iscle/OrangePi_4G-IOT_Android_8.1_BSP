...
public:
     ...
     * shaders sources.
     */
    ANDROID_API Program(const ProgramDescription& description, const char* vertex, const char* fragment);
    ANDROID_API virtual ~Program();

    /**
     * Binds this program to the GL context.
     */
    ANDROID_API virtual void use();

    /**
     * Marks this program as unused. This will not unbind
     * the program from the GL context.
     */
    ANDROID_API virtual void remove();

    /**
     * Returns the OpenGL name of the specified attribute.
     */
    ANDROID_API int getAttrib(const char* name);

    /**
     * Returns the OpenGL name of the specified uniform.
     */
    ANDROID_API int getUniform(const char* name);
     ...
     * transform matrices.
     */
    ANDROID_API void set(const mat4& projectionMatrix, const mat4& modelViewMatrix,
             const mat4& transformMatrix, bool offset = false);

    /**
     * Sets the color associated with this shader.
     */
    ANDROID_API void setColor(FloatColor color);
...
protected:
     ...
     */
    ANDROID_API int addAttrib(const char* name);

    /**
     * Binds the specified attribute name to the specified slot.
     */
    ANDROID_API int bindAttrib(const char* name, ShaderBindings bindingSlot);
     ...
     */
    ANDROID_API int addUniform(const char* name);
...
// private:
     ...
     */
    ANDROID_API GLuint buildShader(const char* source, GLenum type);
