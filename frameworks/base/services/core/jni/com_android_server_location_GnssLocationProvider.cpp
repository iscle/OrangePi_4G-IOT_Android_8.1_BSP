...
#ifdef MTK_GPS_SUPPORT
struct VzwDebugCallback : public IVzwDebugCallback {
    Return<void> vzwDebugCb(const IVzwDebugCallback::VzwDebugData& vzw_message) override;
};

Return<void> VzwDebugCallback::vzwDebugCb(const IVzwDebugCallback::VzwDebugData& vzw_message) {
    ALOGD("vzw_debug_message_callback\n");
    JNIEnv* env = AndroidRuntime::getJNIEnv();
    jstring vzw_msg = env->NewStringUTF((char*)vzw_message.vzw_msg_data.data());
...
#ifdef MTK_GPS_SUPPORT
        auto vzwDebug = gnssHal->getExtensionVzwDebug();
        if (!vzwDebug.isOk()) {
            ALOGD("Unable to get a handle to vzw debug interface");
        } else {
...
static jboolean android_location_GnssLocationProvider_init(JNIEnv* env, jobject obj) {
    ...
    sp<IVzwDebugCallback> vzwDebugCbIface = new VzwDebugCallback();
    if (vzwDebugIface != nullptr) {
        vzwDebugIface->init(vzwDebugCbIface);
    } else {
        ALOGE("Unable to initialize Vzw Debug interface\n");
    }
#endif
    return JNI_TRUE;
}
...
{
    ALOGD("%s: setVzwDebugScreen enabled=%d\n", __func__, enabled);

#ifdef MTK_GPS_SUPPORT
    if (vzwDebugIface == nullptr) {
        return;  // vzw debug screen not support
    }
    vzwDebugIface->setVzwDebugScreen(enabled);
#endif
}
