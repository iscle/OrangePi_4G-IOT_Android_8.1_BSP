...
///M: MTK using location extension by reflection
...
public class LocationManagerService extends ILocationManager.Stub {
    ...
    public void systemRunning() {
        synchronized (mLock) {
            ...
            /// M: init Mtk location manager service ext
    ...
    private int getAllowedResolutionLevel(int pid, int uid) {
        if (mContext.checkPermission(android.Manifest.permission.ACCESS_FINE_LOCATION,
                ...
                /// M: add to check AppOps permission for CTA
    ...
    boolean reportLocationAccessNoThrow(
        ...
        if (op >= 0) {
            if (mAppOps.noteOpNoThrow(op, uid, packageName) != AppOpsManager.MODE_ALLOWED) {
                /// M: add to note AppOps permission for CTA
    ...
    boolean checkLocationAccess(int pid, int uid, String packageName, int allowedResolutionLevel) {
        ...
        if (op >= 0) {
            if (mAppOps.checkOp(op, uid, packageName) != AppOpsManager.MODE_ALLOWED) {
                /// Mtk add to check AppOps permission for CTA
    ...
    private void updateProvidersLocked() {
        ...
        for (int i = mProviders.size() - 1; i >= 0; i--) {
            ...
            } else if (isEnabled && shouldBeEnabled && isWhitelistWorkingMode(name)) {
                mWhitelistWorkingMode = false;
                applyRequirementsLocked(name);
                Log.d(TAG, "Exit whitelist only mode, skip provider enabling and applyRequirement");
            }
            /// M: MTK added log
    ...
    private void updateProviderListenersLocked(String provider, boolean enabled) {
        ...
        if (enabled) {
            ...
            if (mWhitelistProvider.equals(provider)
    ...
    private void requestLocationUpdatesLocked(LocationRequest request, Receiver receiver,
        ...
        /// M: always log requesting location updates
        ...
        if (isProviderEnabled) {
            ...
            /// M: Check if whitelist package wants to use provider when it is disabled
    ...
    private void removeUpdatesLocked(Receiver receiver) {
        /// M: always log removing requests
    ...
    public boolean registerGnssStatusCallback(IGnssStatusListener callback, String packageName) {
        ...
        try {
            ...
            /// M: add to log registered GnssStatus callback packages
    ...
    public void unregisterGnssStatusCallback(IGnssStatusListener callback) {
        synchronized (mLock) {
            try {
                ...
                /// M: add to log unregister GnssStatus callback
    ...
    public boolean isProviderEnabled(String provider) {
        ...
        try {
            synchronized (mLock) {
                LocationProviderInterface p = mProvidersByName.get(provider);
                ///M: mtk added to return true if the provider is in whitelist working mode
                if (p == null) return false;
    ...
    private void handleLocationChangedLocked(Location location, boolean passive) {
        /// M: always log incoming locations
        if (D) {
        ...
        /// M: added coarseLocation by lastLocationCoarse for CTA
        ...
        for (UpdateRecord r : records) {
            ...
            /// M: Skip notifying receivers which are not in whitelist and whitelist working mode
            if (isWhitelistWorkingMode(provider)
                    && !isPackageInWhitelist(receiver.mIdentity.mPackageName)) {
                continue;
            }
            /// M: mtk add end
    ...
    private void handleLocationChanged(Location location, boolean passive) {
        ...
        synchronized (mLock) {
            ...
            /// M: allow whitelist package listeners to receive location changes
    ...
    // whitelist package can use NLP even when provider is disabled
    ...
    private boolean mtkWhitelistRequestLocationUpdates(
        ...
        if (isPackageInWhitelist(packageName) && mWhitelistProvider.equals(name)) {
            if (!mWhitelistWorkingMode) {
                Log.d(TAG, "Enable provider first when first whitelist package requested");
                provider.enable();
    ...
    private boolean mtkCheckCtaOp(int pid, int uid, int op) {
        ///M: if CTA is not enabled, just return true to ignore this check
        if (!isCtaSupported()) {
            return true;
        }
