    ...
    public void onBootPhase(int phase) {
        if (phase == SystemService.PHASE_SYSTEM_SERVICES_READY) {
            ...
            try {
                ...
                Object classObject = (Object) clazzConstructfunc.newInstance(getContext());

                method = mtkIpConnClass.getDeclaredMethod("getMtkIpConnSrv");
                method.setAccessible(true);
                mBinder = (IBinder) method.invoke(classObject);
                if (mBinder == null) {
                ...
                method = mtkIpConnClass.getDeclaredMethod("getNetdEventListener");
                method.setAccessible(true);
                android.net.metrics.INetdEventListener listener =
                        (android.net.metrics.INetdEventListener) method.invoke(classObject);
                if (listener == null) {
                    Log.e(TAG, "mtkIpConnClass's listener is null");
                    return;
                }
                mNetdListener.setMtkNetdEventListener(listener);
            } catch (Exception e) {
