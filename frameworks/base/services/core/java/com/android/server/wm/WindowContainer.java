...
a     * Returns whether this child is on top of the window hierarchy.
    ...
    boolean forAllWindows(ToBooleanFunction<WindowState> callback, boolean traverseTopToBottom) {
        if (traverseTopToBottom) {
            for (int i = mChildren.size() - 1; i >= 0; --i) {
                /// M: Special case, sometimes the element of
                /// mChildren will be removed in mChildren.get(i).forAllWindows(),
                /// so we need check the size again.@{
                if (i >= mChildren.size()) {
                    Slog.w("WindowContainer", "forAllWindows"
                        + ",some child has been removed.mChildren.size():" + mChildren.size()
                        + ",i:" + i);
                    continue;
                }
                /// M: @}
