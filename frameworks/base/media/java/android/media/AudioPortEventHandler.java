...
class AudioPortEventHandler {
    ...
    private static void postEventFromNative(Object module_ref,
        ...
        if (eventHandler != null) {
            ...
            if (handler != null) {
                // [ALPS03051275] Here changes are done to be in sync with AudioPolicyService.
                // Multiple same events came from audio policy, so multiple listAudiopatches()
                // call to audio policy. As audio policy service doing many things, so we gets
                // blocked.
                // To handle this scenario, we have ignore same event if already present in queue.
                if (what == AUDIOPORT_EVENT_PATCH_LIST_UPDATED &&
                        handler.hasMessages(AUDIOPORT_EVENT_PATCH_LIST_UPDATED)) {
                    return;
                }
