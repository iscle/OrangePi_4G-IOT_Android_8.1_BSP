...
public class PerfFrameInfoFactory {
    ...
    public static PerfFrameInfoFactory getInstance() {
        if (null == sInstance) {
            synchronized (lock) {
                if (null == sInstance) {
                    String className = "com.mediatek.perfframe.PerfFrameInfoFactoryImpl";
                    String classPackage = "/system/framework/mediatek-framework.jar";
                    Class<?> clazz = null;
                    try {
                        PathClassLoader classLoader = new PathClassLoader(classPackage,
                                PerfFrameInfoFactory.class.getClassLoader());
                        clazz = Class.forName(className, false, classLoader);
                        Constructor constructor = clazz.getConstructor();
                        sInstance = (PerfFrameInfoFactory) constructor.newInstance();
                    } catch (Exception e) {
