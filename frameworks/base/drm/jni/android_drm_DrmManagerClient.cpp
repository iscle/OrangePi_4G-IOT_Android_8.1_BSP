...
static jobject android_drm_DrmManagerClient_acquireDrmInfo(
    ...
    sp<DrmManagerClientImpl> drmManagerClientImplObject = getDrmManagerClientImpl(env, thiz);

    if(drmManagerClientImplObject != NULL) {
        DrmInfo* pDrmInfo = drmManagerClientImplObject->acquireDrmInfo(uniqueId, &drmInfoReq);
