...
bool populateNameMetadata(const llvm::NamedMDNode *NameMetadata,
  ...
  size_t i = 0;
  for (; i < NameMetadata->getNumOperands(); ++i) {
  ...
  for (auto &GV : module->getGlobalList()) {
    if (!GV.isConstant() && GV.hasInternalLinkage() &&
      ...
      NameList[i] = new char[Name.size() + 1];
      std::string name(Name.data());
      name.append("\0");
      std::memcpy(const_cast<char *>(NameList[i]), name.c_str(), Name.size() + 1);
      ++i;
    }
  }
