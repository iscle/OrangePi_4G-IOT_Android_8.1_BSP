    ...
    while (remainingSize >= 8) {
        ...
        if (!memcmp(chunkHeader, "fmt ", 4)) {
            ...
            if (mValidFormat) {
                ...
                mTrackMeta->setInt32(kKeyEndian, 2);                        // LittleEndian
                mTrackMeta->setInt32(kKeyBitWidth, mBitsPerSample);
                mTrackMeta->setInt32(kKeyPCMType, 1);                   // WAVE_PCM
                if(mBitsPerSample == 8) {
                    mTrackMeta->setInt32(kKeyNumericalType, 2);        // Unsigned data
                } else {
                    mTrackMeta->setInt32(kKeyNumericalType, 1);        // Signed data
                }
#endif
