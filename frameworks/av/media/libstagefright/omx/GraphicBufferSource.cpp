...
status_t GraphicBufferSource::setTimeOffsetUs(int64_t timeOffsetUs) {
    ...
    // for AOSP slowmotion pause-resume issue.
    // MediaCodecSource propagate the timestamp offset to GraphicBufferSource on N1.
    // this Timestamp offset(pause duration) has been  changed by stagefrightrecorder, it's used for outputbuffer
    // timestamp,but GraphicBufferSource use it on Inputbuffer scenario,offset should not be multiplied
    // otherwise,GraphicbufferSource will drop more buffer
    if ((mCaptureFps > 0) && (mFps > 0) && (mCaptureFps > 2*mFps)) {
         mInputBufferTimeOffsetUs = timeOffsetUs/(mCaptureFps/mFps);
    }
    return OK;
}
