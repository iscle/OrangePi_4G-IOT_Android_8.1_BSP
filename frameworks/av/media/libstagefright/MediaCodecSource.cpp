...
//  comment google code
/*
    ALOGD("puller (%s) posting kWhatSetStopTimeUs", mIsAudio ? "audio" : "video");
...
//  add for mtk
    ALOGD("puller (%s) setStopTimeUs", mIsAudio ? "audio" : "video");
...
void MediaCodecSource::Puller::stop() {
    ALOGD("(%s) Puller enter stop", mIsAudio ? "audio" : "video");
    bool interrupt = false;
    {
        ...
        //interruptSource may stuck stop flow, expand duration to 5s from 1s
        interrupt = queue->mReadPendingSince && (queue->mReadPendingSince < ALooper::GetNowUs() - 5000000);
        queue->flush(); // flush any unprocessed pulled buffers
    }

    if (interrupt) {
        ALOGW("[%s] [%d] interruptSource() called, stop flow may stuck", __FUNCTION__, __LINE__);
        interruptSource();
    }
    ALOGD("(%s) Puller stop return", mIsAudio ? "audio" : "video");
}
...
void MediaCodecSource::Puller::handleEOS() {
    ALOGD("puller (%s) posting kWhatPullerNotify EOS", mIsAudio ? "audio" : "video");
...
void MediaCodecSource::Puller::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        ...
        case kWhatSetStopTimeUs:
        {
            ...
            ALOGD("puller (%s) receives kWhatSetStopTimeUs", mIsAudio ? "audio" : "video");
            status_t err = mSource->setStopTimeUs(stopTimeUs);
        ...
        case kWhatStop:
        {
            ALOGD("(%s) Puller receives kWhatStop", mIsAudio ? "audio" : "video");
            mSource->stop();
        ...
        case kWhatPull:
        {
            ...
            if (!queue->mPulling) {
                ALOGD("(%s) Puller handle EOS before read return", mIsAudio ? "audio" : "video");
                handleEOS();
                break;
            }
...
//  add for mtk
//  After calling setStopTimeUs on AudioSource, read may return null
            if (mbuf == NULL && queue->mPulling && mIsAudio) {
                msg->post(30000);
                break;
            }
            if (mbuf == NULL) {
                ALOGD("mPulling=%s, mIsAudio=%s", queue->mPulling?"true":"false", mIsAudio?"true":"false");
            }
//  end for mtk
            queue.unlock();
...
status_t MediaCodecSource::stop() {
    ALOGD("(%s) MediaCodecSource posting kWhatStop", mIsVideo ? "video" : "audio");
...
status_t MediaCodecSource::setStopTimeUs(int64_t stopTimeUs) {
    ALOGD("(%s) MediaCodecSource posting kWhatSetStopTimeUs", mIsVideo ? "video" : "audio");
...
void MediaCodecSource::signalEOS(status_t err) {
    bool reachedEOS = false;
    {
        ...
        if (!reachedEOS) {
            ALOGD("encoder (%s) releasing", mIsVideo ? "video" : "audio");
...
//  add for mtk
        if (!(mFlags & FLAG_USE_SURFACE_INPUT)) {
            ALOGD("Puller (%s) posting kWhatStop", mIsVideo ? "video" : "audio");
            CHECK(mPuller != nullptr);
            mPuller->interruptSource();
            ALOGD("source (%s) stopped", mIsVideo ? "video" : "audio");
        } else {
            ALOGD("Skip stopSource for USE_SURFACE_INPUT");
        }
//  ~add for mtk
...
void MediaCodecSource::onMessageReceived(const sp<AMessage> &msg) {
    ...
    case kWhatPullerNotify:
    {
        ...
        if (msg->findInt32("eos", &eos) && eos) {
            ALOGD("(%s) MediaCodecSource receives kWhatPullerNotify EOS", mIsVideo ? "video" : "audio");
            signalEOS();
            break;
        }
...
//  ~add for mtk
    ...
    case kWhatStop:
    {
        ALOGD("(%s) MediaCodecSource receives kWhatStop", mIsVideo ? "video" : "audio");
        ...
        ALOGD("(%s) MediaCodecSource posting kWhatStopStalled %d", mIsVideo ? "video" : "audio", mGeneration);
        break;
    }
    ...
    case kWhatStopStalled:
    {
        ...
        ALOGD("(%s) MediaCodecSource receives kWhatStopStalled %d", mIsVideo ? "video" : "audio", generation);
...
*/
    ...
    case kWhatSetStopTimeUs:
    {
        ...
        int64_t stopTimeUs;
        CHECK(msg->findInt64("stop-time-us", &stopTimeUs));
        ALOGD("(%s) MediaCodecSource receives kWhatSetStopTimeUs", mIsVideo ? "video" : "audio");
