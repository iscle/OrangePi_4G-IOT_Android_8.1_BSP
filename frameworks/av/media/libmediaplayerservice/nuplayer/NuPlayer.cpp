...
status_t NuPlayer::instantiateDecoder(
    ...
    // mtk add: for debug
    if (mDebugDisableTrackId != 0) {
        if (mDebugDisableTrackId == 1 && audio) {
            ALOGI("Debug: disable audio");
            return -EWOULDBLOCK;
        } else if (mDebugDisableTrackId == 2 && !audio) {
            ALOGI("Debug: disable video");
            return -EWOULDBLOCK;
        }
    }
...
void NuPlayer::getStats(Vector<sp<AMessage> > *mTrackStats) {
    ...
    // mtk add: guard mAudioDecoder, otherwise mAudioDecoder->getStats() may cause timing NE.
    Mutex::Autolock autoLock(mAudioDecoderLock);
    if (mAudioDecoder != NULL) {
