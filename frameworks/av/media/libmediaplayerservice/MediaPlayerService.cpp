...
{
    MM_ALOGV("MediaPlayerService created");
    mNextConnId = 1;
        ...
        audio_session_t audioSessionId)
{
    ...
    MM_ALOGV("Create new client(%d) from pid %d, uid %d, ", connId, pid,
         IPCThreadState::self()->getCallingUid());
...
{
    MM_ALOGV("Client(%d) constructor", connId);
    mPid = pid;
    mConnId = connId;
    mService = service;
    mClient = client;
...
{
    MM_ALOGV("disconnect(%d) from pid %d", mConnId, mPid);
...
#endif
        MM_MLOGI("player type = %d", playerType);
        sp<MediaPlayerBase> p = setDataSource_pre(playerType);
        if (p == NULL) {
            return NO_INIT;
        }

        mStatus = setDataSource_post(p, p->setDataSource(httpService, url, headers));
        setDataSource_drm_proHandle();
        MM_MLOGI("setDataSource(%s) done", url);
        return mStatus;
    }
}
...
{
    MM_ALOGV("setDataSource fd=%d (%s), offset=%lld, length=%lld",
...
{
    MM_ALOGV("[%d] prepareAsync", mConnId);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
...
status_t MediaPlayerService::Client::setNextPlayer(const sp<IMediaPlayer>& player) {
    ...
    if (c != NULL) {
        MM_MLOGI("[%d]setNextPlayer:[%d]",mConnId,mNextClient->mConnId);
        ...
        void* cookie, int msg, int ext1, int ext2, const Parcel *obj)
{
    ...
    if (c != NULL) {
        MM_ALOGV("[%d] notify (%p, %d, %d, %d)", client->mConnId, cookie, msg, ext1, ext2);
...
void MediaPlayerService::Client::setDataSource_drm_proHandle() {
#ifdef MTK_DRM_APP
...
{
    MM_ALOGV("open(%u, %d, 0x%x, 0x%x, %d, %d 0x%x)", sampleRate, channelCount, channelMask,
