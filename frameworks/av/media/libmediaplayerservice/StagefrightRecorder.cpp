...
status_t StagefrightRecorder::setParameters(const String8 &params) {
    ALOGD("setParameters: %s", params.string());
...
#ifdef MTK_SLOW_MOTION_VIDEO_SUPPORT
    if ((mCaptureFps > (2*mFrameRate)) && (mVideoSource == VIDEO_SOURCE_CAMERA)) {
...
status_t StagefrightRecorder::setupMPEG4orWEBMRecording() {
    ...
    if (mVideoSource < VIDEO_SOURCE_LIST_END) {
        ...
        if (err != OK) {
            ALOGE("setupMediaSource Fail err=%d", err);
            return err;
        }
        ...
        err = setupVideoEncoder(mediaSource, &encoder);
        if (err != OK) {
            ALOGE("setupVideoEncoder Fail err=%d", err);
            return err;
        }
...
status_t StagefrightRecorder::resume() {
...
#ifdef MTK_SLOW_MOTION_VIDEO_SUPPORT
    // no need adjust timeoffset for camera hal1 slowmotion
    if (mPersistentSurface != NULL && mCaptureFpsEnable)  {
    ...
    ALOGD("resume() return, resumeStartTimeUs=%lld, pauseStartTimeUs=%lld, compensatory timeoffset=%lld",
        resumeStartTimeUs, mPauseStartTimeUs, (int64_t)timeOffset);
    mPauseStartTimeUs = 0;
    return OK;
}
