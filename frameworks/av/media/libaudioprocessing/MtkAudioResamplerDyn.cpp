    ...
    while (outputIndex < outputSampleCount) {
        ...
        while (mBuffer.frameCount == 0 && inFrameCount > 0) {
            ...
            if (mBuffer.raw == NULL) {
                ...
                mInBuffer.reset();
#if defined(MTK_AUDIO_DEBUG)
                if (AudioDump::getProperty(AudioParameter::PROP_AUDIO_DUMP_RESAMPLER)) {
                    String8 fileName;
                    fileName = String8::format("%s_%p.pcm", AudioParameter::af_resampler_in_pcm, provider);
        ...
        if (inputIndex > 0) {  // we've acquired a buffer (alternatively could check frameCount)
            ...
            if (AudioDump::getProperty(AudioParameter::PROP_AUDIO_DUMP_RESAMPLER)) {
               String8 fileName;
               fileName = String8::format("%s_%p.pcm", AudioParameter::af_resampler_out_pcm, provider);
