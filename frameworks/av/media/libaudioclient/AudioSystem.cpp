...
status_t AudioSystem::setParameters(audio_io_handle_t ioHandle, const String8& keyValuePairs)
{
    ...
    if (param.get(keyhifi_dac, value_str) == NO_ERROR) {
        ...
        if (aps != 0) {
            if (value_str == "on") {
                ret = aps->setPolicyManagerParameters(POLICY_SET_AUDIO_RATE, 192000, 0, 0);
            } else if (value_str == "off") {
                ret = aps->setPolicyManagerParameters(POLICY_SET_AUDIO_RATE, 48000, 0, 0);
            } else {
                ALOGD("setParameters hifi_dac mode error!! use 48k by default");
                ret = aps->setPolicyManagerParameters(POLICY_SET_AUDIO_RATE, 48000, 0, 0);
            }
            return ret;
        }
