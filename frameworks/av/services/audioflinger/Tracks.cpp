...
// MTK_AUDIO>
    ...
    MTK_ALOGI("track(%p): mIsOut %d, mFrameCount %zu, mSampleRate %d, mFormat %d, mChannelCount %d,"
...
void AudioFlinger::ThreadBase::TrackBase::releaseBuffer(AudioBufferProvider::Buffer* buffer)
{
    ...
    if (!mIsOut) {
        if (AudioDump::getProperty(AudioParameter::PROP_AUDIO_DUMP_RECORD)) {
            String8 fileName;
            fileName = String8::format("%s_%d_%p_in.pcm", AudioParameter::af_track_pcm, mTrackCount, this);
...
//#endif
    ...
    MTK_ALOGI("track(%p): mFastIndex %d, mStreamType %d, mName %d",
        ...
        AudioBufferProvider::Buffer* buffer)
{
    ...
    if (NULL == buffer->raw) {
        ...
        if (mIsOut) {
            if (AudioDump::getProperty(AudioParameter::PROP_AUDIO_DUMP_TRACK)) {
                String8 fileName;
                fileName = String8::format("%s_%d_%p_out.pcm", AudioParameter::af_track_pcm, mTrackCount, this);
                AudioDump::threadDump(fileName, buffer->raw, buffer->frameCount * mFrameSize,
                    mFormat, mSampleRate, mChannelCount);
            }
        }
        ...
        int64_t framesWritten, size_t audioHalFrames)
{
    ...
    MTK_ALOGV("presentationComplete() mPresentationCompleteFrames %lld framesWritten %lld",
            (long long)mPresentationCompleteFrames, (long long)framesWritten);
...
void AudioFlinger::RecordThread::RecordTrack::handleSyncStartEvent(const sp<SyncEvent>& event)
{
    if (event == mSyncStartEvent) {
        ...
        ALOGD("%s mFramesToDrop %zd",__FUNCTION__, mFramesToDrop);
    }
}
