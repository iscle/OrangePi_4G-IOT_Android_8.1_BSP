...
status_t AudioFlinger::setParameters(audio_io_handle_t ioHandle, const String8& keyValuePairs)
{
...
#if defined(MTK_AUDIO_DEBUG)
    {
        ...
        String8 value_str;
        int count = AudioParameter::PROP_AUDIO_DUMP_MAXNUM;
        for (int i = 0; i < count; i++) {
            if (param.get(String8(AudioParameter::audioDumpPropertyStr[i]), value_str) == NO_ERROR) {
                MTK_ALOGV("%s set", AudioParameter::audioDumpPropertyStr[i]);
                property_set(AudioParameter::audioDumpPropertyStr[i], value_str);
...
String8 AudioFlinger::getParameters(audio_io_handle_t ioHandle, const String8& keys) const
{
...
#if defined(MTK_AUDIO_DEBUG)
    {
        ...
        int count = AudioParameter::PROP_AUDIO_DUMP_MAXNUM;
        for (int i = 0; i < count; i++) {
            if (keys.compare(String8(AudioParameter::audioDumpPropertyStr[i])) == 0) {
                property_get(AudioParameter::audioDumpPropertyStr[i], value_str, "0");
                String8 ret = String8::format("%s=%s", AudioParameter::audioDumpPropertyStr[i], value_str);
                return ret;
            }
        }
    }
