...
#if defined(MTK_AUDIOMIXER_ENABLE_DRC)
    audio_output_flags_t flagsOriginal = *flags;
#endif
...
void AudioFlinger::PlaybackThread::readOutputParameters_l()
{
    ...
    if (mType == DUPLICATING) {
        if (mFrameCount % 512) {
            MTK_ALOGD("Change Duplicated thread framecount frome %zu to %zu for DRC",
    ...
    if ((mOutput->flags & AUDIO_OUTPUT_FLAG_FAST) && (mOutput->flags & AUDIO_OUTPUT_FLAG_PRIMARY)) {
        ...
        ALOGD("%s, mNormalFrameCount %zu", __FUNCTION__, mNormalFrameCount);
    }
#endif
...
#if defined(MTK_AUDIO_DEBUG)
        if (AudioDump::getProperty(AudioParameter::PROP_AUDIO_DUMP_MIXER)) {
            String8 fileName;
            fileName = String8::format("%s.%d.pid%d.tid%d", AudioParameter::af_mixer_write_pcm, mId, getpid(), gettid());
...
#if defined(MTK_AUDIO_DEBUG)
        if (AudioDump::getProperty(AudioParameter::PROP_AUDIO_DUMP_OFFLOAD)) {
            AudioDump::threadDump(AudioParameter::af_offload_write_raw, (char*)mSinkBuffer + offset, mBytesRemaining,
                mFormat, mSampleRate, mChannelCount );
        }
#endif // MTK_AUDIO_DEBUG
...
#if defined(MTK_AUDIOMIXER_ENABLE_DRC)
    ALOGD("%s, MtkAudioLoud::initParser()+", __FUNCTION__);
    MtkAudioLoud::initParser();
    ALOGD("%s, MtkAudioLoud::initParser()-", __FUNCTION__);
#endif
...
void AudioFlinger::MixerThread::threadLoop_mix()
{
    ...
    if (mMixerBufferValid) {
        if (AudioDump::getProperty(AudioParameter::PROP_AUDIO_DUMP_MIXER)) {
            String8 fileName;
            fileName = String8::format("%s.%d.pid%d.tid%d", AudioParameter::af_mixer_pcm, mId, getpid(), gettid());
    ...
    for (size_t i=0 ; i<count ; i++) {
                ...
                !track->isPaused() && !track->isTerminated())
        {
...
#if defined(MTK_AUDIOMIXER_ENABLE_DRC)
                if (mOutputDevice & AUDIO_DEVICE_OUT_SPEAKER && !track->isOriginalFastTrack()) {
            ...
            if (track->isPausing() || mStreamTypes[track->streamType()].mute) {
                ...
                MTK_ALOGD_IF(vlf != 1.0f || vrf != 1.0f || vh != 1.0f,
                    "vlf %f, vrf %f, vh %f", vlf, vrf, vh);
                vlf *= v * vh;
                vrf *= v * vh;
        ...
        audio_format_t format, audio_session_t sessionId, uid_t uid)
{
    if (trackCountForUid_l(uid) > (PlaybackThread::kMaxTracksPerUid - 1)) {
        MTK_ALOGI("getTrackName_l_FAIL, trackCountForUid_l(%d) = %d", uid, trackCountForUid_l(uid));
        return -1;
    }
...
#if defined(MTK_AUDIO_DEBUG)
                    if (AudioDump::getProperty(AudioParameter::PROP_AUDIO_DUMP_RECORD)) {
                        AudioDump::threadDump(AudioParameter::af_record_pcm, activeTrack->mSink.raw, framesOut * mFrameSize,
                            mFormat, mSampleRate, mChannelCount);
                    }
#endif // MTK_AUDIO_DEBUG
                    if (framesOut > 0) {
