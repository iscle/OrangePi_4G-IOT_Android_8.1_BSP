    ...
    // MTK-END
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                // MTK-START
                /**
                 * Google issue: The new event EVENT_STATE_TIMEOUT added by Google in O-MR1.
                 * There is a 30 seconds timer in Waiting state.
                 * The flow of handling NEW_SMS is NEW_SMS->handle it in Delivering state->
                 * add tracker then switching to Waiting state and waiting for
                 * EVENT_BROADCAST_COMPLETE->Back to IdelState finally once we get
                 * EVENT_BROADCAST_COMPLETE in Waiting state.
                 * The event EVENT_BROADCAST_COMPLETE should be sent when we get broadcast intent
                 * RECEIVED_ACTION in previous version, but we may get it once we get
                 * EVENT_STATE_TIMEOUT in Waiting state after O-MR1.
                 * So we may back to IdleState eariler than before if the system send the broadcast
                 * intent EVENT_BROADCAST_COMPLETE take more thane 30 secs because time out
                 * in Waiting state.
                 * Google change-id: Ib944b28023c1225d6db8945b56eff8996d0a0248
                 */
                case EVENT_BROADCAST_COMPLETE:
                    return HANDLED;
                // MTK-END
    ...
    // MTK-END
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                 ...
                 * Google issue: The new event EVENT_STATE_TIMEOUT added by Google in O-MR1.
                 *
                 * This change will cause Java Exception in DefaultState because of an
                 * unhandled event in DeliveringState. The exception happens in the
                 * following scenario:
                 *   1. New SMS broadcast is completed, EVENT_BROADCAST_COMPLETE is sent
                 *   2. Before EVENT_BROADCAST_COMPLETE is handled, EVENT_STATE_TIMEOUT comes
                 *   3. After handling EVENT_STATE_TIMEOUT in WaitingState, another
                 *      EVENT_BROADCAST_COMPLETE is sent.
                 *   4. The first EVENT_BROADCAST_COMPLETE is handled in WaitingState.
                 *   5. The state is changed to DeliveringState.
                 *   6. The second EVENT_BROADCAST_COMPLETE comes, it will be handled in
                 *      DeliveringState.
                 *   7. The exception happens.
                 *
                 * Google change-id: Ib944b28023c1225d6db8945b56eff8996d0a0248
                 */
                    return HANDLED;
