...
InputDispatcher::InputDispatcher(const sp<InputDispatcherPolicyInterface>& policy) :
    ...
    func = dlsym(handle, "PowerHal_TouchBoost");
    ...
    if (!mConnectionsByFd.isEmpty()) {
        ...
        for (size_t i = 0; i < mConnectionsByFd.size(); i++) {
            ...
            dump.appendFormat(INDENT2 "%zu: channelName='%s', windowName='%s', "
                    ...
                    i, connection->getInputChannelName(), connection->getWindowNameString8().string(),
    ...
    if (dispatchEntry) {
        ...
        if (eventDuration > SLOW_EVENT_PROCESSING_WARNING_TIMEOUT) {
            ...
            msg.appendFormat("Window '%s' spent %0.1fms processing the last input event: ",
                    connection->getWindowNameString8().string(), eventDuration * 0.000001f);
...
void InputDispatcher::traceOutboundQueueLengthLocked(const sp<Connection>& connection) {
    if (ATRACE_ENABLED()) {
        char counterName[40];
        snprintf(counterName, sizeof(counterName), "oq:%s", connection->getWindowNameString8().string());
...
void InputDispatcher::traceWaitQueueLengthLocked(const sp<Connection>& connection) {
    if (ATRACE_ENABLED()) {
        char counterName[40];
        snprintf(counterName, sizeof(counterName), "wq:%s", connection->getWindowNameString8().string());
...
const String8 InputDispatcher::Connection::getWindowNameString8() const {
    if (inputWindowHandle != NULL) {
        return inputWindowHandle->getName();
    }
    if (monitor) {
        return String8("monitor");
    }
    return String8("?");
}
