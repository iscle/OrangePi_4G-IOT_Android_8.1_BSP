...
bool InputConsumer::rewriteMessage(const TouchState& state, InputMessage& msg) {
    ...
    for (uint32_t i = 0; i < msg.body.motion.pointerCount; i++) {
        ...
        if (state.lastResample.idBits.hasBit(id)) {
            ...
            /// M: Need to rewirte UP action after move event has got a resample. @{
            bool isUpAction =
                    (msg.body.motion.action & AMOTION_EVENT_ACTION_MASK) == AMOTION_EVENT_ACTION_UP;
            if (eventTime < state.lastResample.eventTime ||
                    state.recentCoordinatesAreIdentical(id) || isUpAction) {
            /// @}
...
void InputConsumer::resampleTouchState(nsecs_t sampleTime, MotionEvent* event,
    ...
    for (size_t i = 0; i < pointerCount; i++) {
        ...
        if (other->idBits.hasBit(id)
            ...
            char buff[256];
            snprintf(buff, sizeof(buff), "MET_resample: %lld, %lld, %0.6f, %0.6f",
            (long long)sampleTime, (long long)other->eventTime,
            otherCoords.getX(), otherCoords.getY());
            ScopedTrace _resample1(ATRACE_TAG_INPUT, buff);
            snprintf(buff, sizeof(buff), "MET_resample: %lld, %lld, %0.6f, %0.6f",
            (long long)sampleTime, (long long)current->eventTime,
            currentCoords.getX(), currentCoords.getY());
