...
public class PhoneUtils {
    ...
    protected static final boolean DBG = (PhoneGlobals.DBG_LEVEL >= 2);
    ...
    /* package */ protected static String toLogSafePhoneNumber(String number) {
    ...
    static final String EXTENSION_CLASS_NAME = "com.mediatek.phone.MtkPhoneUtils";
    ...
    static CallerInfoToken startGetCallerInfo(Context context, Connection c,
        ...
        try {
            ...
            Class[] argTypes = new Class[] { Context.class, Connection.class,
                    CallerInfoAsyncQuery.OnQueryCompleteListener.class, Object.class,
                    RawGatewayInfo.class };
            Method m = clazz.getDeclaredMethod("startGetCallerInfo", argTypes);
            Object[] params = { context, c, listener, cookie, info };
            log("invoke redirect to " + clazz.getName() + "." + m.getName() + "("
                    + params.toString() + ")");
            return (CallerInfoToken)m.invoke(null, params);
        } catch (Exception e) {
            e.printStackTrace();
            log("startGetCallerInfo invoke redirect fails. Use AOSP instead.");
        }
    ...
    protected static CallerInfoAsyncQuery.OnQueryCompleteListener sCallerInfoQueryListener =
     ...
     */
    /* package */ protected static String modifyForSpecialCnapCases(Context context, CallerInfo ci,
    ...
    public static PhoneAccountHandle makePstnPhoneAccountHandleWithPrefix(
        ...
        try {
            Class clazz = Class.forName(EXTENSION_CLASS_NAME);
            Class[] argTypes = new Class[] { Phone.class , String.class , boolean.class };
            Method m = clazz.getDeclaredMethod("makePstnPhoneAccountHandleWithPrefix", argTypes);
            Object[] params = { phone, prefix, isEmergency };
            log("invoke redirect to " + clazz.getName() + "." + m.getName() + "("
                    + params.toString() + ")");
            return (PhoneAccountHandle)m.invoke(null, params);
        } catch (Exception e) {
            e.printStackTrace();
            log("makePstnPhoneAccountHandleWithPrefix invoke redirect fails. Use AOSP instead.");
        }
    ...
    private static Phone getPhoneFromIccId(String iccId) {
        try {
            ...
            Method m = clazz.getDeclaredMethod("getPhoneFromIccId", argTypes);
            Object[] params = { iccId };
            log("invoke redirect to " + clazz.getName() + "." + m.getName() + "("
                    + params.toString() + ")");
            return (Phone)m.invoke(null, params);
        } catch (Exception e) {
            e.printStackTrace();
            log("getPhoneFromIccId invoke redirect fails. Use AOSP instead.");
        }
