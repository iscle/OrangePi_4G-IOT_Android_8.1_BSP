...
#if 1 //change by jst fro ext audio pa control
//runyee zhou add Speaker_Amp debug begin
static void Ext_Speaker_Amp_Change(bool enable)
{

#if defined(CONFIG_RUNYEE_AWINIC_EXPA_EN)
	#if defined(CONFIG_RUNYEE_AWINIC_EXPA_MODE1) 
		int mode_num,i;    
		mode_num=1;
	#elif defined(CONFIG_RUNYEE_AWINIC_EXPA_MODE2)
		int mode_num,i;    
		mode_num=2;   

	#elif defined(CONFIG_RUNYEE_AWINIC_EXPA_MODE3)
		int mode_num,i;    
		mode_num=3; 

	#elif defined(CONFIG_RUNYEE_AWINIC_EXPA_MODE4) 
		int mode_num,i;    
		mode_num=4;
	#endif
	...
	if (enable)
	{
		...
		{
			AudDrv_GPIO_EXTAMP_Select(0);//GPIO_EXTAMP_LOW
			udelay(1);    //udelay 1us, reality val 7us
			AudDrv_GPIO_EXTAMP_Select(1);//GPIO_EXTAMP_HIGH
	...
	else
	{
		...
		mdelay(1);   //udelay 1us, reality val 7us		  
	}
	
#else
	if (enable)
	{
		//printk("Ext_Speaker_Amp_Change CONFIG_OF ON+\n");
		AudDrv_GPIO_EXTAMP_Select(1);//GPIO_EXTAMP_HIGH
		//AudDrv_GPIO_EXTAMP2_Select(true);
	}
	else 
	{
		//printk("Ext_Speaker_Amp_Change CONFIG_OF  OFF+\n");
		AudDrv_GPIO_EXTAMP_Select(0);//GPIO_EXTAMP_LOW
		//AudDrv_GPIO_EXTAMP2_Select(false);
	}
#endif //CONFIG_RUNYEE_AWINIC_EXPA	
}
//runyee zhou add Speaker_Amp debug end
#else
static void Ext_Speaker_Amp_Change(bool enable)
{
