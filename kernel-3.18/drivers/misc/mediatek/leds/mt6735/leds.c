...
/* add by zhou for runyee pmic isink control begin */
int runyee_isink_set_pmic(enum mt65xx_led_pmic pmic_type, int level)
{
	printk("[RUNYEE LED] PMIC_TYPE = %d, level = %d\n", pmic_type, level);
	mutex_lock(&leds_pmic_mutex);
	...
	if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK0)
	{
		...
		pmic_set_register_value(PMIC_RG_DRV_ISINK0_CK_CKSEL, 0);
		pmic_set_register_value(PMIC_ISINK_CH0_MODE,ISINK_PWM_MODE);
		pmic_set_register_value(PMIC_RG_ISINK0_DOUBLE_EN,0x01); //double current
		pmic_set_register_value(PMIC_ISINK_CH0_STEP,ISINK_5); // 24mA
		pmic_set_register_value(PMIC_ISINK_DIM0_DUTY,31);  //15
		pmic_set_register_value(PMIC_ISINK_DIM0_FSEL,ISINK_1KHZ);//1KHz
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK1)
	{
		...
		pmic_set_register_value(PMIC_RG_DRV_ISINK1_CK_CKSEL, 0);
		pmic_set_register_value(PMIC_ISINK_CH1_MODE,ISINK_PWM_MODE);
	  pmic_set_register_value(PMIC_RG_ISINK1_DOUBLE_EN,0x01); //double current
		pmic_set_register_value(PMIC_ISINK_CH1_STEP,ISINK_5);//16mA
		pmic_set_register_value(PMIC_ISINK_DIM1_DUTY,31);  //15
		pmic_set_register_value(PMIC_ISINK_DIM1_FSEL,ISINK_1KHZ);//1KHz
		if (level) 
		{
			pmic_set_register_value(PMIC_ISINK_CH1_EN,NLED_ON);
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK2)
	{
	  ...
	  pmic_set_register_value(PMIC_RG_ISINK2_DOUBLE_EN,0x01); //double current
		pmic_set_register_value(PMIC_ISINK_CH2_STEP,ISINK_5);//16mA
		pmic_set_register_value(PMIC_ISINK_DIM2_DUTY,31); //15
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK3)
	{
	  ...
	  pmic_set_register_value(PMIC_RG_ISINK3_DOUBLE_EN,0x01); //double current  
		pmic_set_register_value(PMIC_ISINK_CH3_STEP,ISINK_5); // 24mA		
		pmic_set_register_value(PMIC_ISINK_DIM3_DUTY,31); //15
...
#endif //	mt6737 isink2/3 none  
	mutex_unlock(&leds_pmic_mutex);	
	return -1;
}

EXPORT_SYMBOL(runyee_isink_set_pmic);
/* add by zhou for runyee pmic isink control end */ 


#if 1//add by zhou for test start
struct class *pmic_isink_test_class;
struct device *pmic_isink_test_dev;
static ssize_t pmic_isink_test_enable_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    int enable = 0;
    if(buf != NULL && size != 0)
    {
...
static DEVICE_ATTR(pmic_isink_test_enable, 0644, NULL, pmic_isink_test_enable_store);
static int __init pmic_isink_test_init(void)  
{		
	pmic_isink_test_class = class_create(THIS_MODULE, "pmic_isink_test");
	pmic_isink_test_dev = device_create(pmic_isink_test_class,NULL, 0, NULL,  "pmic_isink_test");
    device_create_file(pmic_isink_test_dev, &dev_attr_pmic_isink_test_enable);
    
  //get flashlight dts gpio pin number
#if defined(CONFIG_RUNYEE_LEDS_BUTTON)
  keyled_of_node=of_find_compatible_node(NULL, NULL, "mediatek,flashlight");
  keyled_en_gpio=of_get_named_gpio(keyled_of_node, "keyled_en_gpio", 0);  
  
  gpio_request(keyled_en_gpio, "keyled_en"); 
	gpio_direction_output(keyled_en_gpio,0);	
#endif

	if (NORMAL_BOOT == get_boot_mode())   //zhou add system indicate led
	{		
		runyee_isink_set_pmic(MT65XX_LED_PMIC_NLED_ISINK0,1); 
	} 
...
static void __exit pmic_isink_test_exit(void)
{
	return;
}
module_init(pmic_isink_test_init);
module_exit(pmic_isink_test_exit);
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("pmic_isink_test");
MODULE_AUTHOR("runyee <cheng.zhou@runyee.com.cn>");
