...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
	...
	iReadRegI2C( (u8 *)puSendCmd, 2, (u8 *)&get_byte, 1, imgsensor.i2c_write_id );
...
static void write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
	...
	puSendCmd[0] = (char)(addr >> 8);
	puSendCmd[1] = (char)(addr & 0xFF);
	puSendCmd[2] = (char)(para & 0xFF);
	iWriteRegI2C( (u8 *)puSendCmd, 3, imgsensor.i2c_write_id );
}
...
write_cmos_sensor(0x3820,0x88);  //90-->88    // "88" mirror on    "80" mirror off
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
            printk("zhou1 gpw i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {				
