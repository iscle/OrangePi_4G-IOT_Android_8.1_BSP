...
#if defined(CONFIG_RUNYEE_DOUBLE_IMGSENSOR) //add by jst for main/sub double camera debug
...
extern void mdouble_sensor_init(void);
extern void sdouble_sensor_init(void);
#endif
...
int mtkcam_gpio_init(struct platform_device *pdev)
{
	...
	/*Cam3 Power/Rst Ping initialization */
		       ...
		       char *mode_name)
{
	...
	u32 pinSet[5][8] = {
		...
		{CAMERA_CMRST3_PIN,
		 CAMERA_CMRST3_PIN_M_GPIO,
		 GPIO_OUT_ONE,
		 GPIO_OUT_ZERO,
		 CAMERA_CMPDN3_PIN,
		 CAMERA_CMPDN3_PIN_M_GPIO,
		 GPIO_OUT_ONE,
		 GPIO_OUT_ZERO,
		 },		 
		{GPIO_CAMERA_INVALID,
...
#endif
	printk("%s 111    pinSetIdx:%d, currSensorName: %s\n", __func__, pinSetIdx, currSensorName);


	/* power ON */
	if (On) {
		printk("%s 222    pinSetIdx:%d, currSensorName: %s\n", __func__, pinSetIdx, currSensorName);
		ISP_MCLK1_EN(1);

		printk("[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, currSensorName);

		if ((currSensorName && (0 == strcmp(currSensorName, "gc8024mipiraw"))) 
		   ||(currSensorName && (0 == strcmp(currSensorName, "gc5025mipiraw")))	   
		   ||(currSensorName && (0 == strcmp(currSensorName, "gc2385mipiraw")))	
		   ||(currSensorName && (0 == strcmp(currSensorName, "sp5506mipiraw")))	
		   ) {
		   #if defined(CONFIG_RUNYEE_DOUBLE_IMGSENSOR) //add by jst for main/sub double camera debug
			...
			if (GPIO_CAMERA_INVALID != pinSet[2][IDX_PS_CMPDN])
				mtkcam_gpio_set(2, CAMPDN, pinSet[2][IDX_PS_CMPDN + IDX_PS_ON]);

			if (GPIO_CAMERA_INVALID != pinSet[2][IDX_PS_CMRST])
				mtkcam_gpio_set(2, CAMRST, pinSet[2][IDX_PS_CMRST + IDX_PS_OFF]); 			
		   
			if (GPIO_CAMERA_INVALID != pinSet[3][IDX_PS_CMPDN])
				mtkcam_gpio_set(3, CAMPDN, pinSet[3][IDX_PS_CMPDN + IDX_PS_ON]);

			if (GPIO_CAMERA_INVALID != pinSet[3][IDX_PS_CMRST])
				mtkcam_gpio_set(3, CAMRST, pinSet[3][IDX_PS_CMRST + IDX_PS_OFF]); 
		...
		if ((currSensorName && (0 == strcmp(currSensorName, "gc5025mipiraw")))
	     ||(currSensorName && (0 == strcmp(currSensorName, "gc2385mipiraw")))
	     ||(currSensorName && (0 == strcmp(currSensorName, "sp5506mipiraw")))
		   ) {
		   PK_DBG("[CAMERA SENSOR] open front main/sub sensor gc5025/gc0310 .....\n");
				if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				...
				mtkcam_gpio_set(pinSetIdx, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_ON]);
					...
					if (GPIO_CAMERA_INVALID != pinSet[3][IDX_PS_CMPDN])
					mtkcam_gpio_set(3, CAMPDN, pinSet[3][IDX_PS_CMPDN + IDX_PS_OFF]);	

					if (GPIO_CAMERA_INVALID != pinSet[3][IDX_PS_CMRST])
					mtkcam_gpio_set(3, CAMRST, pinSet[3][IDX_PS_CMRST + IDX_PS_ON]);

					sdouble_sensor_init();
				#endif
		}else{
		PK_DBG("[CAMERA SENSOR] open front main/sub sensor gc8024/gc0310 .....\n");
				if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);	
				...
				mtkcam_gpio_set(pinSetIdx, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_ON]);
				  ...
				  {	
					...
					if (GPIO_CAMERA_INVALID != pinSet[2][IDX_PS_CMPDN])
					{	
						mtkcam_gpio_set(2, CAMPDN, pinSet[2][IDX_PS_CMPDN + IDX_PS_OFF]);	
					}
					
					if (GPIO_CAMERA_INVALID != pinSet[2][IDX_PS_CMRST])
					{	
						mtkcam_gpio_set(2, CAMRST, pinSet[2][IDX_PS_CMRST + IDX_PS_ON]);
					}
