...
#include <asm/ioctl.h>
#include "gt9xx_tpd_custom.h"

#ifdef CONFIG_GTP_GESTURE_WAKEUP
static u16 gesture_key_array[] = TPD_GESTURE_TAB;
...
s8 gtp_enter_doze(void)
{
    ...
    printk("[gt9xx]Entering doze mode.\n");
    while(retry++ < 5) {
        ret = ges_i2c_write_bytes(0x8046, i2c_control_buf, 1);
        if (ret < 0) {
            printk("[gt9xx] failed to set doze flag into 0x8046, %d\n", retry);
            continue;
        }
        ...
        if (!ret){
            ...
            printk("[gt9xx]Gesture mode enabled.\n");
            return ret;
        }
        msleep(10);
    }
...
s32 gesture_event_handler(struct input_dev * dev)
{
	...
	if (DOZE_ENABLED == gesture_data.doze_status) {
		...
		printk("[gt9xx]gesture_event_handler. reg:0x%x = 0x%02X,0x%02X,0x%02X,0x%02X\n", GTP_REG_WAKEUP_GESTURE, doze_buf[0], doze_buf[1], doze_buf[2], doze_buf[3]);

		if (ret == 0 && doze_buf[0] != 0) {
		
#if 1 //runyee zhou add         
       ...
       )
   {
       if (doze_buf[0] != 0x5E)
       {
           GTP_INFO("Wakeup by gesture(%c), light up the screen!", doze_buf[0]);
       }
       else
       {
       ...
       input_report_key(dev, gesture_key_array[type], 1);
       input_sync(dev);
       input_report_key(dev, gesture_key_array[type], 0);
       input_sync(dev);
   ...
   else if ( (doze_buf[0] == 0xAA) || (doze_buf[0] == 0xBB) ||
       (doze_buf[0] == 0xAB) || (doze_buf[0] == 0xBA) )
   {
       char *direction[4] = {"Right", "Down", "Up", "Left"};
       type = ((doze_buf[0] & 0x0F) - 0x0A) + (((doze_buf[0] >> 4) & 0x0F) - 0x0A) * 2;
   ...
   else if (0xCC == doze_buf[0])
   {
       GTP_INFO("Double click to light up the screen!");
	    // gesture_doze_status = DOZE_WAKEUP; 
	   
	     input_report_key(dev, KEY_Z, 1);//KEY_POWER // GESTURE_DOUBLECLICK  to GESTURE_Z key input
       input_sync(dev);
       input_report_key(dev, KEY_Z, 0);//KEY_POWER
       input_sync(dev);
