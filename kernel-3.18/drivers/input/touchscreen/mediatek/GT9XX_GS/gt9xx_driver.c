...
#ifdef CONFIG_GTP_GESTURE_WAKEUP
	//printk("[gt9xx] touch_event_handler gesture_data.enabled=%d\n",gesture_data.enabled); //zhou
...
{
	int retval;
#ifdef CONFIG_GTP_GESTURE_WAKEUP //runyee zhou add	
	int idx;
#endif	
...
#ifdef CONFIG_GTP_GESTURE_WAKEUP //runyee zhou add
   for (idx = 0; idx < GTP_GESTURE_MAX_KEY_NUM; idx++)
    {
        input_set_capability(tpd->dev, EV_KEY, gesture_key_array[idx]);
        __set_bit(gesture_key_array[idx], tpd->dev->keybit);
    }  
#endif	
	
	input_set_abs_params(tpd->dev, ABS_MT_TRACKING_ID, 0, (GTP_MAX_TOUCH-1), 0, 0);
	
#ifdef CONFIG_TPD_HAVE_BUTTON //runyee zhou add
	if (tpd_dts_data.use_tpd_button) {
...
static s8 gtp_wakeup_sleep(struct i2c_client *client)
{
	...
	while (retry++ < 10) {
#ifdef CONFIG_GTP_GESTURE_WAKEUP
	//printk("[gt9xx] gtp_wakeup_sleep gesture_data.enabled=%d\n",gesture_data.enabled); //zhou
	//if (gesture_data.enabled) {
	if (1) {
...
#ifdef CONFIG_GTP_GESTURE_WAKEUP
	//printk("[gt9xx] tpd_suspend gesture_data.enabled=%d\n",gesture_data.enabled); //zhou
	//if (gesture_data.enabled) {
	if (1) {
		ret = gtp_enter_doze();
		gtp_irq_enable();
